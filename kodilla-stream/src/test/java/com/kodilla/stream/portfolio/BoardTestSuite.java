...
public class BoardTestSuite {
    ...
    void testAddTaskListFindUsersTasks() {
        ...
        List<Task> tasks = project.getTaskLists().stream()
                ...
                .filter(t -> t.getAssignedUser().equals(user)).toList();
    ...
    void testAddTaskListFindOutdatedTasks() {
        ...
        List<Task> tasks = project.getTaskLists().stream()
                ...
                .filter(t -> t.getDeadline().isBefore(LocalDate.now())).toList();
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        var taskTimeSpentList = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
                .map(date -> ChronoUnit.DAYS.between(date, LocalDate.now()))
                .toList();
        var sum = taskTimeSpentList.stream()
                .reduce(0L, (s, current) -> s = s + current);
        var count = (long) taskTimeSpentList.size();
        double avg = (double) sum /count;

        assertEquals(10.0, (double) sum / count);
